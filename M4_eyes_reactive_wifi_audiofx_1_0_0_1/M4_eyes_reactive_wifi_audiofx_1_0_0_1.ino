...
void setup() {
...
#else
  if(!arcada.filesysBegin())    fatal("No filesystem found!", 2500);
#endif
  ...
  // config1.eye, config2.eye or config3.eye instead).
  ...
  if((digitalRead(pin1) == HIGH) && arcada.exists("config1.eye")) {
    filename = (char *)"config1.eye";
  } else if((digitalRead(pin2) == HIGH) && arcada.exists("config2.eye")) {
    filename = (char *)"config2.eye";
  } else if((digitalRead(pin3) == HIGH) && arcada.exists("config3.eye")) {
  ...
  if(!eye[eyeNum].column_ready) {
    if(!x) { // If it's the first column...
      ...
      if((t - timeOfLastBlink) >= timeToNextBlink) { // Start new blink?
        ...
        timeToNextBlink = blinkDuration * 3 + random(8000000);
      }
      ...
      frames++;
      if(((t - lastFrameRateReportTime) >= 5000000) && t) { // Once per sec.
...
void batt_check(){
  float batt_lev = analogRead(battery);
  batt_lev = batt_lev * 0.005882;
  //batt_lev = 2.5; //for testing
  Serial.print("Battery level: ");
  Serial.println(batt_lev);
...
void low_batt(){
  ...
  if (showSplashScreen) {
    ...
    showSplashScreen = ((arcada.drawBMP((char *)"/batt.bmp",
                         0, 0, eye[0].display)) == IMAGE_SUCCESS);
    if (showSplashScreen) { // Loaded OK?
      Serial.println("Low Battery!");
      if (NUM_EYES > 1) {   // Load on other eye too, ignore status
        yield();
        arcada.drawBMP((char *)"/batt.bmp", 0, 0, eye[1].display);
      }
        for (int i = 0; i < 10000; i++){
          float batt_lev = analogRead(battery);
          batt_lev = batt_lev * 0.005882;
          Serial.println(batt_lev);
      // Ramp up backlight over 2 sec duration
          startTime = millis();
          while ((elapsed = (millis() - startTime)) <= 2000) {
            yield();
            arcada.setBacklight(255 * elapsed / 2000);
          }
          delay(1000);
      
          uint8_t blb = 254;
          while(blb > 0x27){
            arcada.setBacklight(blb);
            yield();
            blb--;
            delay(50);
          }
          ...
          if ((batt_lev > (analogRead(battery) * 0.005882)) && (i>7)){
            idle();
          }
        }
      }
  ...
  // If no splash, or load failed, turn backlight on early so user gets a
  // little feedback, that the board is not locked up, just thinking.
  if (!showSplashScreen) arcada.setBacklight(0);
...
void idle(){
  ...
  while(true){
    delay(60000);
    Serial.printf("Idle battery level: %f \n", (analogRead(battery) * 0.005882));
    if ((analogRead(battery) * 0.005882) > 3.2) return;
    Serial.printf("Idle for %d minutes.\n", i);
    i++;
  }
}
