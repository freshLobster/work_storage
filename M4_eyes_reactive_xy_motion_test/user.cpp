...
bool direction = 0; //switch direction
...
void user_loop(void) {
  ...
  float rand = (float)random(-500, 500);
  rand += (float)random(-500, 500);
  rand += (float)random(-500, 500);
  rand = rand/3000;
  Serial.println(rand);
  if (t > 200){
    if(direction){
      eyeTargetX += 0.1;
      eyeTargetY += rand;
      if (eyeTargetY < -0.5) eyeTargetY = -0.5;
      if(eyeTargetY > 0.5) eyeTargetY = 0.5;
    }else{
      eyeTargetX -= 0.1;
      eyeTargetY += rand;
    ...
    if (eyeTargetX >= 0.5 || eyeTargetX <= -0.5){
    direction = !direction;
    }
  }
